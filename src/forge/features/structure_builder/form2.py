...
class StructureBuilderForm(ida_kernwin.PluginForm):
    ...
    def OnCreate(self, form):
        ...
        # Table
        self.ui.table.cellDoubleClicked.connect(self.structure_table_interaction)
        self.ui.table.itemChanged.connect(self.structure_table_item_changed)

        # Actions
        self.ui.action_enable.triggered.connect(lambda: self.structure_table_enable_row(None))
        self.ui.table.addAction(self.ui.action_enable)
        self.ui.action_disable.triggered.connect(lambda: self.structure_table_disable_row(None))
        self.ui.table.addAction(self.ui.action_disable)
        self.ui.action_resolve.triggered.connect(self.structure_table_resolve)
        self.ui.table.addAction(self.ui.action_resolve)
        self.ui.action_finalize.triggered.connect(self.structure_table_finalize)
        self.ui.table.addAction(self.ui.action_finalize)

        # Context menu
        # Structure List
        self.ui.lst_structures.setContextMenuPolicy(Qt.CustomContextMenu)
        self.ui.lst_structures.customContextMenuRequested.connect(self.list_context_menu)

        # Structure Table
        self.ui.table.setContextMenuPolicy(Qt.CustomContextMenu)
        self.ui.table.customContextMenuRequested.connect(self.structure_table_context_menu)
    ...
    def update_structure_fields(self):
        if self.current_structure is None:
            self.ui.table.setDisabled(True)
            self.ui.structure_name.setText("")
            self.ui.table.setRowCount(0)
            self.current_structure = None
        else:
            self.current_structure.refresh_collisions()
            self.ui.table.setEnabled(True)
            self.ui.structure_name.setText(self.current_structure.name)

            self.ui.table.setRowCount(len(self.current_structure.members))
            for row, member in enumerate(self.current_structure.members):
                self.ui.table.setItem(row, column.offset,
                                      QTableWidgetItem(f"0x{member.offset:04X} [{hex(member.size)}]"))
                self.ui.table.setItem(row, column.type, QTableWidgetItem(member.type_name))
                self.ui.table.setItem(row, column.name, QTableWidgetItem(member.name))
                self.ui.table.setItem(row, column.score, QTableWidgetItem(member.score))
                self.ui.table.setItem(row, column.comment, QTableWidgetItem(member.comment))

                # Color table
                if self.current_structure.main_offset == member.offset:
                    self.ui.table.item(row, column.offset).setBackground(
                        QColor(config['form']['origin_color']))

                if not member.enabled:
                    set_row_background_color(self.ui.table, row,
                    ...
                    set_row_foreground_color(self.ui.table, row,
                                             QColor(config['form']['collision_foreground_color']))

    def structure_renamed(self):
        name = self.ui.structure_name.toPlainText()
    ...
    def structure_table_context_menu(self, point):
        item = self.ui.table.itemAt(point)
        if item is None:
            return

        row = item.row()

        self.ui.action_enable.triggered.connect(lambda: self.structure_table_enable_row(row))
        self.ui.action_disable.triggered.connect(lambda: self.structure_table_disable_row(row))
        ...
        menu.exec_(self.ui.table.viewport().mapToGlobal(point))
