...
forge_api / ida MCP). Resolved items are archived at the bottom with
one-line summaries (details live in git history).
...
- **Status**: open. **Live-reproduced 2026-08-12 on an idalib 9.4 worker**
...
- **Status**: open. Live-reproduced with the R10 probe:
...
- **Status**: open. Every step that maps a recovered type onto code required
...
- **Status**: open. `__int64 a1` root → **2 members**; same function
...
- **Status**: open. Without `create_structure(name)` first they raise
...
- **Status**: open (caller-side workaround exists and works).
...
- **Status**: open. Members disabled by collisions are excluded from
...
- **Status**: open — the read-side twin of I9/I11.
...
- **Status**: open (agent-session gap, 2026-08-12).
...
- **Status**: open — the session's own goal ("infer what functions are
...
- **Status**: open (only the write side exists: `to_vtable`).
...
- **Status**: open — "does this binary use malloc/new?" needs one call;
...
- **Status**: open — every decompile was sliced by hand
...
- **Status**: open (tiny) — existence guard before `create_type`/`to_vtable`
  (the session used raw `tinfo_t().get_named_type` in bisects).
...
- **Status**: open — `to_usercall` rewrites signatures only as a
...
- **Status**: open — the requested "finds the allocation and scans from
...
- **Status**: open — `_extract_numeric_argument` returns 0 for any non-num
...
- **Status**: open — `grid_chain = build_grid_chain(2, 3u)` →
...
- **Status**: open — the DB's `charNode` committed **empty**
...
- **Status**: open — the chicken-egg: `add_member("GridNode", 0,
...
- **Status**: open (engine gaps, live 2026-08-12). (1) `scan_global` on an
...
- **Status**: open — two parallel in-memory stores both holding
...
- **Status**: open (requested 2026-08-12: "auto parse all types into
...
```bash
python -m pytest -q            # full suite (455 passing)
